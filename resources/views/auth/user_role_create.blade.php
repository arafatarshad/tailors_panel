  ...
  <section class="content">
    ...
    <div class="box box-default">
      ...
      {!! Form::open(['method'=>'POST', 'action'=>['PermissionController@add_user_role'], 'id'=>'add-new-customer-form']) !!} 
                  {{ csrf_field() }}      
      <div class="box-body">
        <h2>User info</h2>
        <div class="row">
            ...
            <div class="box-body">
              ...
              <div class="form-group">
                ...
                @foreach($all_roles as $all_role)
                  @if($all_role->name!=null)
                  <li>
                    <label>
                      @if(isset($roles) AND $roles->role_id==$all_role->id)
                      <input type="radio"  value="{{$all_role->id}}" name="role" checked>
                          {{$all_role->name}}
                      @else
                      <input type="radio"  value="{{$all_role->id}}" name="role" >
                          {{$all_role->name}}
                      @endif
